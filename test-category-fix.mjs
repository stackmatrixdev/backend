...
async function testCategoryValidation() {
  try {
    ...
    console.log(
      "📤 Sending request with category:",
      testTrainingData.programData.category
    );
...
async function testInvalidCategory() {
  ...
  try {
    ...
    if (
      ...
      result.error &&
      result.error.errors &&
      result.error.errors.category
    ) {
      console.log(
        '✅ EXPECTED: Invalid category "Technology" correctly rejected'
      );
      console.log("🔍 Error message:", result.error.errors.category.message);
    } else {
      console.log(
        "⚠️  Unexpected: Invalid category was accepted or other error occurred"
      );
    }
  } catch (error) {
...
console.log(
  '- Removed invalid categories like "Technology", "Financial", etc.'
);
